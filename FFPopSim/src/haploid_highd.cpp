 ...
 */
haploid_highd::haploid_highd(int L_in, int rng_seed, int n_o_traits, bool all_polymorphic_in, bool most_polymorphic_in) {
	...
	most_polymorphic=most_polymorphic_in;
...
int haploid_highd::allocate_mem() {
	...
	mutant_loci = new int [number_of_mutant_loci];
...
int haploid_highd::mutate() {
	...
	int tmp_individual=0, nmut=0, n_o_muts=0, mut_locus=0;
	size_t mutant;
	allele_frequencies_up_to_date = false;
	int actual_n_o_mutations,actual_n_o_mutants;
	if (mutation_rate > HP_NOTHING and not all_polymorphic and not most_polymorphic) {
	...
	} else if(most_polymorphic) {
		if(HP_VERBOSE) cerr <<"haploid_highd::mutate(): keeping most loci polymorphic"<<endl;
		calc_allele_freqs(); //calculate the allele frequencies
		nmut=0;
		mut_locus=0;				
		for (int locus=0; locus<L(); locus++){	//loop over all loci
			if (locus==mutant_loci[mut_locus]){
				n_o_muts = gsl_ran_poisson(evo_generator, (1.0-exp(-mutation_rate))*population_size);
				produce_random_sample(min(n_o_muts, population_size));

				//make sure enough empty clones are available to accomodate the new mutants
				provide_at_least(min(n_o_muts, population_size));

				//loop over the mutant individuals and introduce the mutations
				for (int individual = 0; individual != n_o_muts; individual++) {
					//determine the target clone
					mutant = random_clone();
					//determine the number of mutation it suffers. this should be Poisson conditional on having at least one
					//in practice the solution is fine but it is somewhat inaccurate for multiple mutations
					actual_n_o_mutations = gsl_ran_poisson(evo_generator, mutation_rate)+1;
					//introduce the mutations, not that flip_single_locus returns the number of new mutant, which is fed back into
					//flip_single_locus to introduce the next mutation
					for (int i = 0; i != actual_n_o_mutations; i++)
