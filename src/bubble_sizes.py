...
pop.add_trait_coefficient(deleterious_effects[0],[mutant_loc],1)
...
if __name__ == '__main__':
    ...
    plt.figure()
    w_theor_bins = np.logspace(0,6,20)
    w_2 = w_theor_bins**-2
    ...
    data = np.histogram(bubble_size_sums,bins=logbins,range=(1,1e6))
    plt.plot(data[1][:-1],data[0],label='$\sigma$ = '+str(sigma) + ', $\delta$ = '+str(delta))
