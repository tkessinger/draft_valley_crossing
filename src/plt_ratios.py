...
# N_range = [10**3,10**4,10**5]#,3*10**4,10**5,3*10**5,10**6] #range of population size values
# sigma_range = [5*10**-6,5*10**-2]
# delta_range = [0.0,10**-4,10**-2] #range of deleterious intermediate coefficients
# s_range_valley = [10**-2] #range of selection coefficients for the adaptation
# s_range_sweep = [10**-2]
# mu_range = [5*10**-5]
# numruns = 3
# rho=0.0
...
shifted_cmap = shiftedColorMap(orig_cmap, midpoint=(0.8), name='shifted')
...
axes = grid
im = axes[0].imshow(np.log10((array3/array4)/(array1/array2)),cmap=shifted_cmap,vmin=-0.8,vmax=0.2)
axes[0].set_xticklabels(xticks)
axes[0].set_yticklabels(yticks)
axes[0].set_xlabel(r'$\delta$')
axes[0].set_ylabel(r'$N$')
...
orig_cmap = matplotlib.cm.Greens
...
axes = grid
im = axes[0].imshow(np.log10(array1),cmap=orig_cmap,vmin=2,vmax=4)
axes[0].set_xticklabels(xticks)
axes[0].set_yticklabels(yticks)
axes[0].set_xlabel(r'$\delta$')
axes[0].set_ylabel(r'$N$')
axes[0].set_title(r'$\sigma_1 = 5 \times 10^{-6}$')
im = axes[1].imshow(np.log10(array3),cmap=orig_cmap,vmin=2,vmax=4)
axes[1].set_xticklabels(xticks)
axes[1].set_yticklabels(yticks)
axes[1].set_xlabel(r'$\delta$')
axes[1].set_ylabel(r'$N$')
...
cb = axes[1].cax.colorbar(im)
axes[1].cax.toggle_label(True)
cb.set_label_text(r'$\log_{10} \tau_{\mathrm{valley}}$')
#plt.tight_layout()    # Works, but may still require rect parameter to keep colorbar labels visible
plt.show()
