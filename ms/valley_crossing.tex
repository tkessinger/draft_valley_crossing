...
\iffalse
In this case, the exact values of $U_b$ and $\bar{\epsilon}$ turn out not to matter: what is relevant is the product $D = U_b \bar{\epsilon}^2/2$.
\fi
...
\begin{equation}
v\partial_x p(x,s) = x p(x,s) + \frac{U_b\bar{\epsilon}^2}{2}\partial_x^2 p(x,s) - \frac{p(x,s)^2}{2},
\end{equation}
...
In this regime, $x_c \approx 2\sigma^4/U_b\bar{\epsilon}^2$: when $\sigma$ is large, only mutations in the nose stand any chance of fixing unless they can be dragged to higher fitness by a background mutations (large $D$).
...
At high values of $\sigma$, the fitness effect of the deleterious intermediate mutation has very little effect on the valley crossing time, which means that very deep valleys can be crossed.
This suggests that traversing rugged fitness landscapes by valley crossing may be enhanced in rapidly adapting populations: as previously mentioned, they appear to be ``flattened".
...
We can get a clearer picture of how draft affects valley crossing times relative to single beneficial sweeps by considering the ratio $\alpha(10^{-2})/\alpha(10^{-6})$, which is a measure of the extent to which rapid adaptation speeds up valley crossing relative to sweeping (lower values indicate that rapid adaptation favors valley crossing).
...
For $N > 10^5$ in the low $\sigma$ case, the product $N\mu > 1$: tunneling becomes semi-deterministic and, on average, more than one mutant lineage is produced every generation.
These mutants then give rise to double mutants at a low but constant rate that is not sensitive to the valley depth (Figure \ref{fig:tau-ratios} and equation 35 in \citealp{weissman_2009}) and that is much faster than in the stochastic tunneling case.
In contrast, $N > 10^{5}$ for high $\sigma$ does not produce the same order of magnitude increase in valley crossing rates, which is likely due to no analogous qualitative change in valley crossing dynamics.
Thus, for large enough population sizes, draft does not significantly improve the valley crossing rate relative to the rate of single beneficial sweeps.
\iffalse
\jcomment{Double check my explanation here :-)}
