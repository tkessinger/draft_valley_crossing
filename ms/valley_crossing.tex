...
Genetic loci are partitioned into two groups, the two ``focal" loci where the epistatic alleles $A$ and $BB$ will segregate and the remaining ``background" loci that are responsible for the underlying fitness distribution.
...
In this way, the mutation rate $U$ \jcomment{Is this $U_{B}$ or $U$?} and average fitness effect $\bar{\epsilon}$ at the background loci \jcomment{this should be defined clearly either above in this section or in the math background section (it appears in that section but is not explicitly defined)} are not parameters of our simulation model but must be directly measured from simulations; i.e., they are constrained by $L$ and $\sigma$.
...
After equilibration, we set a per-site mutation rate $\mu$ for the focal loci (allowing forward and backward mutations) with single mutant fitness disadvantage $\delta$ and double mutant advantage $s$.
...
We first studied the effect of increasing $\sigma$ in the sequential fixation, stochastic tunneling, semi-deterministic tunneling, and sequential fixation regimes \citep{weissman_2009}.
\jcomment{commented out some text here; put back into the results if the pattern described is evident there}
...
It also requires that $\ll 1/\mu$ \jcomment{$N$ missing here?}, meaning that only one mutant lineage is likely to be extant at any given time and, if lineages do co-occur, they are unlikely to interfere with each other or substantially affect the mean fitness.
This happens to be the necessary condition for a birth-death process to be an appropriate model.\jcomment{is this previous sentence necessary?}
...
In general, we expect valley crossing to require more time in rapidly adapting populations (large $N \sigma$) experiencing genetic draft as bubbles are doomed to extinction more quickly \jcomment[inline]{I was going to add something like, ``(bubbles of intermediate size are less likely to occur under draft)'' to explain this, but is it more general (since you bring it up only under tunneling below)? Maybe add a short parenthetical explanation?} and beneficial lineages are less likely to fix, especially when valleys are shallow.
...
The underlying dynamics are more complicated, however.\jcomment{Yes, but the following sentences don't highlight specifically enough I think why; e.g., what about a simple reduction in $N_{e}$ cannot explain what happens in this draft case?}
As $\sigma$ increases, the effect of the genetic background in which an mutation finds itself becomes more significant in determining the fate of that mutation than the fitness effect of the mutation itself.
This genetic draft buffets the frequency of mutations so that these mutations behave as though they are more neutral than they really are.
...
We can get a clearer picture of how draft affects valley crossing times relative to single beneficial sweeps by plotting the ratio $\alpha(10^{-2})/\alpha(10^{-6})$ in Figure \ref{fig:alpha_ratio}, which is a measure of the extent to which rapid adaptation speeds up valley crossing relative to sweeping (lower values indicate that rapid adaptation favors valley crossing).
Overall, we find that increasing $\sigma$ increases the extent to which valley crossing is favored for $N < 10^{5}$. In this regime, increasing $\delta$ also increases the degree to which valley crossing is favored under draft, which means that the deeper the valley, the more that draft increases the likelihood of valley crossing relative to a single beneficial mutation.
For $N > 10^5$ in the low $\sigma$ case, tunneling becomes semi-deterministic where $N\mu > 1$ and, on average, more than one mutant lineage is generated every generation.\jcomment{Another figure presumably from simulations that you're running...}
These mutants then give rise to double mutants at a low but constant rate that is not sensitive to the valley depth (Figure \ref{fig:tau-ratios} and equation 35 in \citealp{weissman_2009}) and that is much faster than in the stochastic tunneling case. In contrast, $N > 10^{5}$ for high $\sigma$ does not produce the same order of magnitude increase in valley crossing rates, which is likely due to no analogous qualitative change in valley crossing dynamics.
Thus, for large enough population sizes, draft does not significantly improve the valley crossing rate relative to the rate of single beneficial sweeps.\jcomment{Double check my explanation here :-)}

At high $\sigma$, the number of intermediate mutants is likely to fluctuate substantially \citep{Cvijovic:Good:2018}, but the average behavior is more or less the same.
This relation more or less obtains in a rapidly adapting population as well, with substantial fluctuations, so $\pfix$ becomes more important in determining the valley crossing rate: since this scales more weakly with $s$ under draft, the overall crossing rate is slowed.\jcomment{I still don't get what the point of mentioning this is here; how does this connection to the behavior of the ratio of $\alpha$s?}
...
We find that although the overall valley crossing time is longer in rapidly adapting than in slowly adapting (drifting) populations, a higher fraction of the adaptation that occurs rapidly adapting populations will occur through valley crossing than in slowly adapting populations.
...
They find that large populations are more likely to cross valley than fix a simple beneficial mutation than small populations (for comparison, see the $\sigma=5 \times 10^{-6}$ panel in our Figure \ref{fig:ratios}).
...
Thus, it is possible that genetic draft improves the likelihood of valley crossing precisely where its least likely under genetic drift. 
% in their model, the complicating factor is that a partially sweeping beneficial mutation increases the mean fitness, making valley crossing more difficult especially for populations of intermediate size.
\jcomment[inline]{I removed an argument had here about mean fitness that I wasn't sure of; how does the sweeping beneficial mutation increasing mean fitness affect intermediate population sizes more than large or small?}
