...
In this regime, $x_c \approx 2\sigma^4/U_b\bar{\epsilon}^2$: when $\sigma$ is large, only mutations in the nose stand any chance of fixing unless they can be dragged to higher fitness by a background mutations.
The effect of introducing a nonzero $s$ is essentially to shift the value of $x$ and $x_c$: for large negative $s$, $x_c$ may be far enough in the nose of the Gaussian distribution that it is unlikely any individuals will have a high enough background fitness to fix, and for large positive $s$, even individuals with an unfit genetic background can give rise to a lineage that is destined to fix.
...
At high values of $\sigma$, the fitness effect of the deleterious intermediate mutation has very little effect on the valley crossing time, which means that deep valleys can be more easily crossed.
This suggests that the ``flattening'' of rugged fitness landscapes due to genetic draft may in fact yield some benefits to valley crossing in rapidly adapting populations easier.
...
For $N > 10^5$ in the low $\sigma$ case, tunneling becomes semi-deterministic where $N\mu > 1$ and, on average, more than one mutant lineage is produced every generation.
